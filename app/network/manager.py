...
class NetworkManager(QObject):
    ...
    def _run(self) -> None:
        self.status_changed.emit("Ricerca del server in corso...")
    ...
    def _elect_server(self) -> None:
        ...
        # In caso di accensione quasi simultanea, Medico 1
        # ha una breve precedenza. Un piccolo jitter evita
        # collisioni perfettamente simultanee.
        base_delay = 0.25 if self.doctor_id == "doctor1" else 0.85
        time.sleep(base_delay + random.uniform(0.05, 0.20))

        if self._stop_event.is_set():
            return

        # Ricontrolliamo dopo l'attesa: nel frattempo l'altro
        # computer potrebbe essere diventato Server.
        server_address = self._discover_server()
        ...
        if self._start_server_services():
            self._set_role("server", self._get_local_ip())
        ...
        # Sullo stesso computer un'altra istanza potrebbe aver
        # occupato la porta un attimo prima di noi.
        self.status_changed.emit(
    ...
    def _start_server_services(self) -> bool:
        ...
        try:
            # Nessun SO_REUSEADDR: sullo stesso PC la porta
            # deve appartenere a un solo Server.
        ...
        self._heartbeat_socket = heartbeat_socket
    ...
    def _discovery_server_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                raw_request, sender = discovery_socket.recvfrom(4096)
            except socket.timeout:
                continue
            except OSError:
                break

            request = self._decode_message(raw_request)

            if request.get("type") != DISCOVERY_MESSAGE:
                continue
    ...
    def _heartbeat_server_loop(self) -> None:
        heartbeat_socket = self._heartbeat_socket
        ...
        while not self._stop_event.is_set():
            try:
                connection, _ = heartbeat_socket.accept()
            except socket.timeout:
                continue
            except OSError:
                break
            ...
            with connection:
                connection.settimeout(HEARTBEAT_TIMEOUT_SECONDS)
                ...
                if request.get("type") != HEARTBEAT_MESSAGE:
                    continue
        ...
        try:
            with socket.socket(
                ...
                while (
                    ...
                    server_ip = str(
                        response.get("server_ip") or sender[0]
                    )

                    # Se stiamo testando due istanze sullo stesso PC,
                    # il loopback è il percorso più affidabile.
    ...
    def _run_as_client(self, server_address: str) -> None:
        self._set_role("client", server_address)
        self.status_changed.emit(
        ...
        while not self._stop_event.is_set():
            if self._ping_server(server_address):
            ...
            if missed_heartbeats >= MAX_MISSED_HEARTBEATS:
                ...
                # Prima di promuoverci, verifichiamo se esiste
                # già un altro Server sulla rete.
                replacement_server = self._discover_server()

                if replacement_server:
                    server_address = replacement_server
                    self._set_role("client", server_address)
                    missed_heartbeats = 0
                    continue
    ...
    def _ping_server(self, server_address: str) -> bool:
        request = {
    ...
    def _set_role(
        ...
        self.server_address_changed.emit(server_address)
    ...
    def _get_local_ip() -> str:
        try:
            with socket.socket(
                ...
                return str(ip_socket.getsockname()[0])
        except OSError:
            return "127.0.0.1"
